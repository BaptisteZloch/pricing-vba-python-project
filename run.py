...
with open(
    f"./benchmarks/result_gap_convergence_{mkt.spot_price}_{mkt.volatility}_{mkt.interest_rate}_{mkt.dividend_price}_{opt.option_type}_{opt.exercise_type}_{opt.strike_price}.csv",
    mode="w",
) as f:
...
for step in tqdm(range(1, 100, 10), desc="Computing convergence and GAP"):
    ...
    start_time = time()
    price = t.price(opt=opt, draw_tree=False)
    end_time = time()
    with open(
        f"./benchmarks/result_gap_convergence_{mkt.spot_price}_{mkt.volatility}_{mkt.interest_rate}_{mkt.dividend_price}_{opt.option_type}_{opt.exercise_type}_{opt.strike_price}.csv",
        mode="a",
